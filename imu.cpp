...
void processAcc(){
    ...
    //attitude (angle) estimation
     float roll = (atan2(accelG.x, accelG.z)*180)/M_PI; // -180° --> 180°
     float pitch = (atan2(accelG.y, accelG.z)*180)/M_PI; // -180° --> 180°
     //yaw = (atan(accelG.z / sqrt(sq(accelG.y) + sq(accelG.x))))*(360/(2*M_PI));  

     imuCombine(accel_x_filter, accel_y_filter);
    
  }


void imuCombine(median_filter_t accel_x_filter, median_filter_t accel_y_filter){
   ...
   angle.z = 1 * (angle.z + (gyroRates.z * delta_t));
